...
import { CLASSES, SEASONS, GAME_MODES, TIER_CONFIG } from "@/lib/constants";
...
type TierListResponse = Record<string, (Build & { score: number; ratio: number; tier: string })[]>;
...
export default function HomePage() {
  ...
  const { data: tierList, isLoading } = useQuery<TierListResponse>({
    queryKey: ["/api/tier-list", seasonId, gameMode],
    queryFn: async () => {
      const params = new URLSearchParams({ seasonId, gameMode });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3" data-testid="filters-section">
        <Select value={seasonId} onValueChange={setSeasonId}>
          <SelectTrigger className="w-[220px] bg-card border-border" data-testid="select-season">
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {(["S", "A", "B", "C", "D"] as const).map((tier) => {
            const builds = filteredTierList[tier] || [];
            ...
            return (
              <div key={tier} className="space-y-3" data-testid={`tier-section-${tier}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className={`text-sm font-semibold ${config.color.split(" ").pop()}`}>
                      {config.description}
                    </span>
          ...
          {Object.values(filteredTierList).every((arr) => arr.length === 0) && (
            <div className="text-center py-16 text-muted-foreground" data-testid="text-empty-state">
              ...
              <p className="text-sm mt-1">
                Try a different season, game mode, or class filter.
              </p>
            </div>
          )}
        </div>
